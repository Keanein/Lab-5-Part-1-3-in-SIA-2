<?php
    ...
    $stmt->close();
?>
<h1 class="mb-4">Category: <?php echo htmlspecialchars($categoryName); ?></h1>
            ...
            card.innerHTML = `
                            <div class="card-header-custom">
                                <div class="user-avatar">
                                    ${post.user.profile_pic ? `<img src="${post.user.profile_pic}" style="width: 100%; height:100%; border-radius: 50%; object-fit: cover;">` : '<i class="bi bi-person"></i>'}
                                </div>
                            ...
                            <div class="card-actions">
                                <div class="action-buttons">
                                    <button class="action-btn vote-btn" data-post-id="${post.id}" data-vote="1"><i class="bi bi-arrow-up"></i> <span class="vote-count up-${post.id}">${post.upvotes}</span></button>
                                    <button class="action-btn vote-btn" data-post-id="${post.id}" data-vote="-1"><i class="bi bi-arrow-down"></i> <span class="vote-count down-${post.id}">${post.downvotes}</span></button>
                                    <a href="<?php echo BASE_URL;?>/post.php?id=${post.id}" class="action-btn comment-link"><i class="bi bi-chat"></i> <span class="vote-count">${post.comments}</span></a>
                                </div>
