<?php
// latest.php - PHASE 5 PROFESSOR'S VERSION
// Shows newest memes first (similar to index but explicitly "Latest")

require_once __DIR__ . '/includes/config.php';
require_once __DIR__ . '/includes/header.php';

// Get initial posts - newest first
$limit = 10;
$offset = 0;
...
<div class="row g-4">
    ...
    <div class="col-lg-6">
        ...
        <div class="profile-header mb-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h2 class="mb-0">Latest Memes</h2>
                    <p class="text-muted mb-0">Newest memes first • <?= $total_posts ?> total memes</p>
                </div>
            </div>
        </div>
            ...
            <?php if (count($initial_posts) === $limit): ?>
                <div class="load-more-wrapper">
                    <button class="load-more-btn" id="load-more-btn" data-last-id="<?= $last_post_id ?>">
                        <i class="bi bi-arrow-repeat"></i> Load More Latest Memes
                    </button>
                </div>
            <?php endif; ?>
        <?php endif; ?>
    ...
    <div class="col-lg-3">
        ...
        <div class="sidebar-card mt-3">
            <div class="sidebar-title">
                <i class="bi bi-info-circle"></i> ABOUT LATEST
            </div>
            <div class="category-list">
                <div class="category-link" style="cursor: default;">
                    <span class="emoji">🕒</span> Newest memes appear here first
                </div>
                <div class="category-link" style="cursor: default;">
                    <span class="emoji">📊</span> <?= $total_posts ?> memes in total
                </div>
                <div class="category-link" style="cursor: default;">
                    <span class="emoji">❤️</span> Vote for your favorites
                </div>
            </div>
        </div>
    </div>
        ...
        try {
            ...
            if (posts.length > 0) {
                ...
                if (posts.length < 10 || newPostsCount === 0) {
                    ...
                    this.disabled = false;
                    this.innerHTML = '<i class="bi bi-arrow-repeat"></i> Load More Latest Memes';
                }
            } else {
                this.style.display = 'none';
            }
